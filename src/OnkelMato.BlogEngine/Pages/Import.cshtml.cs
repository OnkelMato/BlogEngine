...
public class ImportModel(
    BlogEngineRepository repository,
    ...
    IOptionsMonitor<ImportExportConfiguration> imexConfiguration) : PageModel
{
    ...
    public async Task<IActionResult> OnPostAsync()
    {
        ...
        if (clearBlog)
            await repository.ClearPostsAndImages();
        ...
        await repository.DoImportBlog(importModel);
    ...
    private async Task<string?> ImportJsonFromJwt(string token)
    {
        ...
        foreach (var certificateFile in imexConfiguration.CurrentValue.CertificateFile)
        {
            try
            {
    ...
    {
        var result = false;
        foreach (var certificate in imexConfiguration.CurrentValue.CertificateFile)
        {
